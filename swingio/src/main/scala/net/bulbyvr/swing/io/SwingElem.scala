...
final class MainElem[F[_]] private[io] (val fullConstructor: Resource[F, (swingio.MainFrame[F], Deferred[F, Unit])])(using F: Async[F]) 
  extends SwingElem[F, swingio.MainFrame[F], (swingio.MainFrame[F], Deferred[F, Unit])] {
  override def apply[M](modifier: M)(using M: Modifier[F, swingio.MainFrame[F], M]): Resource[F, (swingio.MainFrame[F], Deferred[F, Unit])] =
    fullConstructor.flatTap((l, _) => M.modify(modifier, l))
  override def withSelf[M](mkModifier: swingio.MainFrame[F] => M)(using M: Modifier[F, swingio.MainFrame[F], M]): Resource[F, (swingio.MainFrame[F], Deferred[F, Unit])] =
...
private trait SwingElems[F[_]](using F: Async[F]) {
  ...
  lazy val box = 
    NormalElem[F, swingio.BoxPanel[F]](swingio.BoxPanel[F](swingio.Orientation.Vertical))
